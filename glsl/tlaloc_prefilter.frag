...
layout(set = 0, binding = 1) uniform textureCube samplerEnv;
...
vec3 prefilterEnvMap(vec3 R, float roughness) {
  ...
  float envMapDim = float(textureSize(samplerCube(samplerEnv, sampler0), 0).s);
  for (uint i = 0u; i < consts.numSamples; i++) {
    ...
    if (dotNL > 0.0) {
      ...
      color += textureLod(samplerCube(samplerEnv, sampler0), L, mipLevel).rgb *
               dotNL;
      totalWeight += dotNL;
    }
  }
