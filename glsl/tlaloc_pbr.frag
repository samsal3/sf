...
layout(set = 3, binding = 0) uniform textureCube samplerIrradiance;
layout(set = 3, binding = 1) uniform textureCube prefilteredMap;
layout(set = 3, binding = 2) uniform texture2D samplerBRDFLUT;

// Material bindings

layout(set = 2, binding = 0) uniform sampler sampler0;
layout(set = 2, binding = 1) uniform texture2D colorMap;
layout(set = 2, binding = 2) uniform texture2D physicalDescriptorMap;
...
// Encapsulate the various inputs used by the various functions in the shading
// equation We store values in this struct to simplify the integration of
// alternative implementations of the shading terms, outlined in the Readme.MD
// Appendix.
struct PBRInfo {
...
vec3 getNormal() {
  ...
  vec3 tangentNormal = texture(sampler2D(normalMap, sampler0),
...
// source. Precomputed Environment Maps are required uniform inputs and are
...
vec3 getIBLContribution(PBRInfo pbrInputs, vec3 n, vec3 reflection) {
  ...
  vec3 brdf =
      (texture(sampler2D(samplerBRDFLUT, sampler0),
               vec2(pbrInputs.NdotV, 1.0 - pbrInputs.perceptualRoughness)))
          .rgb;
  vec3 diffuseLight =
      SRGBtoLINEAR(
          tonemap(texture(samplerCube(samplerIrradiance, sampler0), n)))
          .rgb;

  vec3 specularLight =
      SRGBtoLINEAR(tonemap(textureLod(samplerCube(prefilteredMap, sampler0),
                                      reflection, lod)))
          .rgb;
...
float convertMetallic(vec3 diffuse, vec3 specular, float maxSpecular) {
  ...
  float b = perceivedDiffuse * (1.0 - maxSpecular) / (1.0 - c_MinRoughness) +
            perceivedSpecular - 2.0 * c_MinRoughness;
...
void main() {
  ...
  if (material.alphaMask == 1.0f) {
    if (material.baseColorTextureSet > -1) {
      baseColor = SRGBtoLINEAR(texture(
                      sampler2D(colorMap, sampler0),
  ...
  if (material.workflow == PBR_WORKFLOW_METALLIC_ROUGHNESS) {
    ...
    if (material.physicalDescriptorTextureSet > -1) {
      ...
      vec4 mrSample =
          texture(sampler2D(physicalDescriptorMap, sampler0),
  ...
  if (material.workflow == PBR_WORKFLOW_SPECULAR_GLOSINESS) {
    ...
    vec4 diffuse = SRGBtoLINEAR(texture(sampler2D(colorMap, sampler0), inUV0));
    vec3 specular =
        SRGBtoLINEAR(texture(sampler2D(physicalDescriptorMap, sampler0), inUV0))
            .rgb;
  ...
  vec3 v =
      normalize(ubo.camPos - inWorldPos); // Vector from surface point to camera
  ...
  if (material.occlusionTextureSet > -1) {
    float ao = texture(sampler2D(aoMap, sampler0),
  ...
  if (material.emissiveTextureSet > -1) {
    vec3 emissive =
        SRGBtoLINEAR(texture(sampler2D(emissiveMap, sampler0),
  ...
  if (ubo.debugViewInputs > 0.0) {
    ...
    case 1:
      outColor.rgba =
          material.baseColorTextureSet > -1
              ? texture(sampler2D(colorMap, sampler0),
                        material.baseColorTextureSet == 0 ? inUV0 : inUV1)
              : vec4(1.0f);
      break;
    case 2:
      outColor.rgb =
          (material.normalTextureSet > -1)
              ? texture(sampler2D(normalMap, sampler0),
                        material.normalTextureSet == 0 ? inUV0 : inUV1)
                    .rgb
              : normalize(inNormal);
      break;
    case 3:
      outColor.rgb =
          (material.occlusionTextureSet > -1)
              ? texture(sampler2D(aoMap, sampler0),
                        material.occlusionTextureSet == 0 ? inUV0 : inUV1)
                    .rrr
              : vec3(0.0f);
      break;
    case 4:
      outColor.rgb =
          (material.emissiveTextureSet > -1)
              ? texture(sampler2D(emissiveMap, sampler0),
                        material.emissiveTextureSet == 0 ? inUV0 : inUV1)
                    .rgb
              : vec3(0.0f);
      break;
    case 5:
      outColor.rgb =
          texture(sampler2D(physicalDescriptorMap, sampler0), inUV0).bbb;
      break;
    case 6:
      outColor.rgb =
          texture(sampler2D(physicalDescriptorMap, sampler0), inUV0).ggg;
      break;
    }
