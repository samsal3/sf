...
SF_INTERNAL SfVulkanQueueFamilyPropertyList sfVulkanCreateQueueFamilyPropertyList(SfArena *arena, SfGraphicsRenderer *renderer) {
...
SF_INTERNAL void sfVulkanFindQueueFamilyIndices(SfGraphicsRenderer *renderer) {
   ...
   for (uint32_t i = 0; i < list.count && !sfVulkanValidateQueueFamilyIndices(renderer); ++i) {
      ...
      vkGetPhysicalDeviceSurfaceSupportKHR(renderer->vulkanPhysicalDevice, i, renderer->vulkanSurface, &supportsSurface);
...
SF_INTERNAL SfVulkanExtensionPropertyList sfVulkanCreateExtensionPropertyList(SfArena *arena, SfGraphicsRenderer *renderer) {
   SfVulkanExtensionPropertyList result = {0};

   if (SF_VULKAN_CHECK(vkEnumerateDeviceExtensionProperties(renderer->vulkanPhysicalDevice, NULL, &result.count, NULL))) {
      result.data = sfAllocate(arena, result.count * sizeof(*result.data));
      if (result.data) {
         SF_VULKAN_CHECK(vkEnumerateDeviceExtensionProperties(renderer->vulkanPhysicalDevice, NULL, &result.count, result.data));
      }
   }
...
SF_INTERNAL SfBool sfVulkanCheckDeviceExtensionSupport(SfGraphicsRenderer *renderer) {
   ...
   for (uint32_t i = 0; i < renderer->vulkanDeviceExtensionCount && foundAllExtensions; ++i) {
      ...
      for (uint32_t j = 0; j < list.count && !foundCurrentExtension; ++j) {
         ...
         foundCurrentExtension = sf_s8_compare(currentExtension, currentAvailableExtension, VK_MAX_EXTENSION_NAME_SIZE);
      }
...
SF_INTERNAL SfBool sfVulkanCheckDeviceSwapchainSupport(SfGraphicsRenderer const *renderer) {
   ...
   SF_VULKAN_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(renderer->vulkanPhysicalDevice, renderer->vulkanSurface, &surfaceFormatCount, NULL));
   SF_VULKAN_CHECK(vkGetPhysicalDeviceSurfacePresentModesKHR(renderer->vulkanPhysicalDevice, renderer->vulkanSurface, &presentModeCount, NULL));
...
SF_INTERNAL SfVulkanSurfaceFormatList sfVulkanCreateSurfaceFormatList(SfArena *arena, SfGraphicsRenderer *renderer) {
   ...
   if (SF_VULKAN_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(renderer->vulkanPhysicalDevice, renderer->vulkanSurface, &result.count, NULL))) {
      result.data = sfAllocate(arena, result.count * sizeof(*result.data));
      if (result.data) {
         SF_VULKAN_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(renderer->vulkanPhysicalDevice, renderer->vulkanSurface, &result.count, result.data));
      }
   }
...
SF_INTERNAL SfVulkanPresentModeList sfVulkanCreatePresentModeList(SfArena *arena, SfGraphicsRenderer *renderer) {
   ...
   if (SF_VULKAN_CHECK(vkGetPhysicalDeviceSurfacePresentModesKHR(renderer->vulkanPhysicalDevice, renderer->vulkanSurface, &result.count, NULL))) {
      result.data = sfAllocate(arena, result.count * sizeof(*result.data));
      if (result.data) {
         SF_VULKAN_CHECK(vkGetPhysicalDeviceSurfacePresentModesKHR(renderer->vulkanPhysicalDevice, renderer->vulkanSurface, &result.count, result.data));
      }
   }
...
SF_INTERNAL uint32_t sfVulkanFindMemoryTypeIndex(VkPhysicalDevice device, VkMemoryPropertyFlags memoryProperties, uint32_t filter) {
   ...
   if (device) {
      ...
      for (uint32_t currentType = 0; currentType < availableProperties.memoryTypeCount && result == (uint32_t)-1; ++currentType)
         if ((filter & (1 << currentType)) && (availableProperties.memoryTypes[currentType].propertyFlags & memoryProperties) == memoryProperties)
            result = currentType;
   }
...
SF_INTERNAL VkDeviceMemory sfAllocateVulkanMemory(SfGraphicsRenderer *renderer, VkMemoryPropertyFlags memoryProperties, uint32_t filter, uint64_t count) {
   VkDeviceMemory memory = VK_NULL_HANDLE;

   uint32_t memoryTypeIndex = sfVulkanFindMemoryTypeIndex(renderer->vulkanPhysicalDevice, memoryProperties, filter);
   if (memoryTypeIndex != (uint32_t)-1) {
      ...
      SF_VULKAN_CHECK(vkAllocateMemory(renderer->vulkanDevice, &info, renderer->vulkanAllocationCallbacks, &memory));
   }

   return memory;
}

SF_INTERNAL VkDeviceMemory sfVulkanAllocateAndBindMemoryForImage(SfGraphicsRenderer *renderer, VkImage image, VkMemoryPropertyFlags properties) {
   VkDeviceMemory memory = VK_NULL_HANDLE;
...
SF_INTERNAL VkDeviceMemory sf_vulkan_allocate_and_bind_memory_to_buffer(SfGraphicsRenderer *renderer, VkBuffer buffer, VkMemoryPropertyFlags properties) {
   VkDeviceMemory memory = VK_NULL_HANDLE;
...
void sfGraphicsDeviceWaitIdle(SfGraphicsRenderer const *renderer) {
   vkDeviceWaitIdle(renderer->vulkanDevice);
}
...
SF_INTERNAL void sfDestroySwapchainResources(SfGraphicsRenderer *renderer) {
   ...
   if (renderer->vulkanDevice && renderer->vulkanSwapchain) {
      vkDestroySwapchainKHR(renderer->vulkanDevice, renderer->vulkanSwapchain, renderer->vulkanAllocationCallbacks);
...
SF_INTERNAL void sfCreateGraphicsSwapchainResources(SfGraphicsRenderer *renderer) {
   uint32_t const queueFamilyIndices[] = {renderer->graphicsQueue.vulkanQueueFamilyIndex, renderer->presentQueue.vulkanQueueFamilyIndex};

   VkSurfaceCapabilitiesKHR capabilities = {0};
   vkGetPhysicalDeviceSurfaceCapabilitiesKHR(renderer->vulkanPhysicalDevice, renderer->vulkanSurface, &capabilities);
   ...
   if (SF_VULKAN_CHECK(vkCreateSwapchainKHR(renderer->vulkanDevice, &swapchainInfo, renderer->vulkanAllocationCallbacks, &renderer->vulkanSwapchain))) {
      if (SF_VULKAN_CHECK(vkGetSwapchainImagesKHR(renderer->vulkanDevice, renderer->vulkanSwapchain, &renderer->vulkanSwapchainImageCount, NULL))) {
         if (SF_SIZE(renderer->vulkanSwapchainImages) < renderer->vulkanSwapchainImageCount) {
            ...
            SF_VULKAN_CHECK(vkGetSwapchainImagesKHR(renderer->vulkanDevice, renderer->vulkanSwapchain, &renderer->vulkanSwapchainImageCount, renderer->vulkanSwapchainImages));
         }
      }
   }
...
SfBool sfGraphicsQueueSubmitGraphicsCommand(SfGraphicsRenderer *renderer, SfHandle queueHandle, uint32_t commandBufferCount, SfHandle const *commandBufferHandles, uint32_t waitSemaphoreCount, SfHandle const *waitSemaphoreHandles, uint32_t signalSemaphoreCount, SfHandle const *signalSemaphoreHandles) {
   SfBool result = SF_FALSE;
   if (renderer && queueHandle) {
      ...
      for (uint32_t i = 0; i < commandBufferCount; i++)
         vulkanCommandBuffers[i] = ((SfGraphicsCommandBuffer const *)commandBufferHandles[i])->vulkanCommandBuffer;

      for (uint32_t i = 0; i < waitSemaphoreCount; i++)
         vulkanWaitSemaphores[i] = ((SfGraphicsSemaphore const *)waitSemaphoreHandles[i])->vulkanSemaphore;

      for (uint32_t i = 0; i < waitSemaphoreCount; i++)
         vulkanStageFlags[i] = VK_PIPELINE_STAGE_ALL_COMMANDS_BIT;

      for (uint32_t i = 0; i < signalSemaphoreCount; i++)
         vulkanSignalSemaphores[i] = ((SfGraphicsSemaphore const*)signalSemaphoreHandles[i])->vulkanSemaphore;
...
SfBool sfGraphicsQueuePresent(SfGraphicsRenderer *renderer, SfHandle queueHandle, uint32_t waitSemaphoreCount, SfHandle const *waitSemaphoreHandles) {
   SfBool result = SF_FALSE;
...
void sfDestroyGraphicsTexture(SfGraphicsRenderer *renderer, SfHandle textureHandle) {
   if (renderer && textureHandle) {
      ...
      if (texture->vulkanImageView) {
         vkDestroyImageView(renderer->vulkanDevice, texture->vulkanImageView, renderer->vulkanAllocationCallbacks);
         texture->vulkanImageView = VK_NULL_HANDLE;
      }

      if (texture->ownsImage) {
         if (texture->vulkanMemory) {
            vkFreeMemory(renderer->vulkanDevice, texture->vulkanMemory, renderer->vulkanAllocationCallbacks);
         }
         if (texture->vulkanImage) {
            vkDestroyImage(renderer->vulkanDevice, texture->vulkanImage, renderer->vulkanAllocationCallbacks);
         }
      }
...
void sfDestroyGraphicsSemaphore(SfGraphicsRenderer *renderer, SfHandle semaphoreHandle) {
   if (renderer && semaphoreHandle) {
      ...
      if (renderer->vulkanDevice && semaphore->vulkanSemaphore) {
         vkDestroySemaphore(renderer->vulkanDevice, semaphore->vulkanSemaphore, renderer->vulkanAllocationCallbacks);
         semaphore->vulkanSemaphore = VK_NULL_HANDLE;
      }
   }
}
...
SfHandle sfCreateGraphicsSemaphore(SfGraphicsRenderer *renderer) {
   ...
   if (semaphore) {
      ...
      if (!SF_VULKAN_CHECK(vkCreateSemaphore(renderer->vulkanDevice, &info, renderer->vulkanAllocationCallbacks, &semaphore->vulkanSemaphore))) {
...
void sfDestroyGraphicsFence(SfGraphicsRenderer *renderer, SfHandle fenceHandle) {
   if (renderer && fenceHandle) {
      ...
      if (renderer->vulkanDevice && fence->vulkanFence) {
         vkDestroyFence(renderer->vulkanDevice, fence->vulkanFence, renderer->vulkanAllocationCallbacks);
         fence->vulkanFence = VK_NULL_HANDLE;
      }
   }
}
...
SfHandle sfCreateGraphicsFence(SfGraphicsRenderer *renderer) {
   ...
   if (fence) {
      ...
      if (!SF_VULKAN_CHECK(vkCreateFence(renderer->vulkanDevice, &info, renderer->vulkanAllocationCallbacks, &fence->vulkanFence))) {
...
void sfDestroyGraphicsCommandPool(SfGraphicsRenderer *renderer, SfHandle commandPoolHandle) {
   if (renderer && commandPoolHandle) {
      ...
      if (renderer->vulkanDevice && commandPool->vulkanCommandPool) {
         vkDestroyCommandPool(renderer->vulkanDevice, commandPool->vulkanCommandPool, renderer->vulkanAllocationCallbacks);
         commandPool->vulkanCommandPool = VK_NULL_HANDLE;
      }
   }
}

SfHandle sfCreateGraphicsCommandPool(SfGraphicsRenderer *renderer, SfHandle queueHandle, SfBool transient, SfBool reset) {
   SfGraphicsCommandPool *commandPool = sfGetOrAllocateCommandPool(renderer);
   if (commandPool) {
      SfGraphicsQueue *queue = (SfGraphicsQueue *)queueHandle;
      ...
      if (!SF_VULKAN_CHECK(vkCreateCommandPool(renderer->vulkanDevice, &info, renderer->vulkanAllocationCallbacks, &commandPool->vulkanCommandPool))) {
...
void sfDestroyGraphicsCommandBuffer(SfGraphicsRenderer *renderer, SfHandle commandPoolHandle, SfHandle commandBufferHandle) {
   if (renderer && commandPoolHandle && commandBufferHandle) {
      ...
      if (renderer->vulkanDevice && commandPool->vulkanCommandPool && commandBuffer->vulkanCommandBuffer) {
         vkFreeCommandBuffers(renderer->vulkanDevice, commandPool->vulkanCommandPool, 1, &commandBuffer->vulkanCommandBuffer);
         commandBuffer->vulkanCommandBuffer = VK_NULL_HANDLE;
      }
   }
}
...
SfHandle sfCreateGraphicsCommandBuffer(SfGraphicsRenderer *renderer, SfHandle commandPoolHandle, SfBool secondary) {
   ...
   if (commandBuffer) {
      ...
      if (!SF_VULKAN_CHECK(vkAllocateCommandBuffers(renderer->vulkanDevice, &info, &commandBuffer->vulkanCommandBuffer))) {
...
SfHandle sfCreateGraphicsTexture(SfGraphicsRenderer *renderer, SfGraphicsTextureDescription const *description) {
   ...
   if (texture) {
      ...
      if (description->vulkanNotOwnedImage) {
         ...
         if (SF_VULKAN_CHECK(vkCreateImage(renderer->vulkanDevice, &info, renderer->vulkanAllocationCallbacks, &texture->vulkanImage))) {
            VkMemoryPropertyFlags flags = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT;

            if (texture->mapped) 
               flags |= VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;

            texture->vulkanMemory = sfVulkanAllocateAndBindMemoryForImage(renderer, texture->vulkanImage, flags);
            if (texture->vulkanMemory && texture->mapped) {
               SF_VULKAN_CHECK(vkMapMemory(renderer->vulkanDevice, texture->vulkanMemory, 0, VK_WHOLE_SIZE, 0, &texture->mappedData));
            }
         }
      }

      if (texture->vulkanImage && (texture->ownsImage ? !!texture->vulkanMemory : SF_TRUE) && (texture->mapped ? !!texture->mappedData : SF_TRUE)) {
         ...
         if (SF_VULKAN_CHECK(vkCreateImageView(renderer->vulkanDevice, &info, renderer->vulkanAllocationCallbacks, &texture->vulkanImageView))) {
            texture->vulkanAspectFlags = info.subresourceRange.aspectMask;
            texture->vulkanLayout = (SF_GRAPHICS_TEXTURE_USAGE_STORAGE & texture->usage) == SF_GRAPHICS_TEXTURE_USAGE_STORAGE ? VK_IMAGE_LAYOUT_GENERAL : VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
         }
      }


      if (!texture->vulkanImage || !(texture->ownsImage ? !!texture->vulkanMemory : SF_TRUE) ||  !(texture->mapped ? !!texture->mappedData : SF_TRUE) || !texture->vulkanImageView) {
   ...
   if (renderer && renderTargetHandle) {
      ...
      if (renderer->vulkanDevice) {
         if (renderTarget->vulkanFramebuffer) {
            vkDestroyFramebuffer(renderer->vulkanDevice, renderTarget->vulkanFramebuffer, renderer->vulkanAllocationCallbacks);
            renderTarget->vulkanFramebuffer = VK_NULL_HANDLE;
         }

         if (renderTarget->vulkanRenderPass) {
            vkDestroyRenderPass(renderer->vulkanDevice, renderTarget->vulkanRenderPass, renderer->vulkanAllocationCallbacks);
            renderTarget->vulkanRenderPass = VK_NULL_HANDLE;
         }
      }
...
SfHandle sfCreateGraphicsRenderTarget(SfGraphicsRenderer *renderer, SfGraphicsRenderTargetDescription const *description) {
   ...
   if (renderTarget) {
      ...
      if (renderTarget->samples == SF_GRAPHICS_SAMPLE_COUNT_1) {
         ...
         attachmentDescriptions = sfAllocate(arena, totalAttachmentCount * sizeof(*attachmentDescriptions));
         colorAttachmentReferences = sfAllocate(arena, renderTarget->colorAttachmentCount * sizeof(*colorResolveAttachmentReferences));
         ...
         if (attachmentDescriptions && colorAttachmentReferences && attachmentImageViews) {
            for (uint32_t i = 0; i < renderTarget->colorAttachmentCount; ++i) {
               ...
               SfGraphicsClearValue const *clearValue = &description->colorAttachmentClearValues[i];
               ...
               renderTarget->colorAttachments[i] = sfCreateGraphicsTexture(renderer, &textureDescription);
               if (renderTarget->colorAttachments[i]) {
                  attachmentDescriptions[colorIndex].flags = 0;
                  attachmentDescriptions[colorIndex].format = sfAsVulkanFormat(renderTarget->colorFormat);
                  attachmentDescriptions[colorIndex].samples = VK_SAMPLE_COUNT_1_BIT;
                  attachmentDescriptions[colorIndex].loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
                  attachmentDescriptions[colorIndex].storeOp = VK_ATTACHMENT_STORE_OP_STORE;
                  attachmentDescriptions[colorIndex].stencilLoadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
                  attachmentDescriptions[colorIndex].stencilStoreOp = VK_ATTACHMENT_STORE_OP_STORE;
                  attachmentDescriptions[colorIndex].initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
                  if (renderTarget->vulkanSwapchainImage)
                     attachmentDescriptions[colorIndex].finalLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
                  else
                     attachmentDescriptions[colorIndex].finalLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
                  ...
                  attachmentImageViews[colorIndex] = ((SfGraphicsTexture *)renderTarget->colorAttachments[i])->vulkanImageView;
               }
            }
         }
      } else {
         ...
         attachmentDescriptions = sfAllocate(arena, totalAttachmentCount * sizeof(*attachmentDescriptions));
         colorAttachmentReferences = sfAllocate(arena, renderTarget->colorAttachmentCount * sizeof(*colorAttachmentReferences));
         colorResolveAttachmentReferences = sfAllocate(arena, renderTarget->colorAttachmentCount * sizeof(*colorAttachmentReferences));
         attachmentImageViews = sfAllocate(arena, totalAttachmentCount * sizeof(*attachmentImageViews));
         if (attachmentDescriptions && colorAttachmentReferences && colorResolveAttachmentReferences && attachmentImageViews) {
            for (uint32_t i = 0; i < renderTarget->colorAttachmentCount; ++i) {
               ...
               renderTarget->colorAttachments[i] = sfCreateGraphicsTexture(renderer, &colorAttachmentDescription);
               ...
               renderTarget->colorMultisampleAttachments[i] = sfCreateGraphicsTexture(renderer, &resolveAttachmentDescription);
               if (renderTarget->colorAttachments[i] && renderTarget->colorMultisampleAttachments[i]) {
                  attachmentDescriptions[colorIndex].flags = 0;
                  attachmentDescriptions[colorIndex].format = sfAsVulkanFormat(renderTarget->colorFormat);
                  attachmentDescriptions[colorIndex].samples = VK_SAMPLE_COUNT_1_BIT;
                  attachmentDescriptions[colorIndex].loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
                  ...
                  attachmentDescriptions[resolve_index].format = sfAsVulkanFormat(renderTarget->colorFormat);
                  attachmentDescriptions[resolve_index].samples = sfAsVulkanSampleCount(renderTarget->samples);
                  attachmentDescriptions[resolve_index].loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
                  attachmentDescriptions[resolve_index].storeOp = VK_ATTACHMENT_STORE_OP_STORE;
                  attachmentDescriptions[resolve_index].stencilLoadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
                  attachmentDescriptions[resolve_index].stencilStoreOp = VK_ATTACHMENT_STORE_OP_STORE;
                  attachmentDescriptions[resolve_index].initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
                  attachmentDescriptions[resolve_index].finalLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;

                  colorAttachmentReferences[i].attachment = colorIndex;
                  colorAttachmentReferences[i].layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;

                  colorResolveAttachmentReferences[i].attachment = resolve_index;
                  colorResolveAttachmentReferences[i].layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;

                  attachmentImageViews[colorIndex] = ((SfGraphicsTexture *)renderTarget->colorAttachments[i])->vulkanImageView;
                  attachmentImageViews[resolve_index] = ((SfGraphicsTexture *)renderTarget->colorMultisampleAttachments[i])->vulkanImageView;
               }
            }
         }
      }
      ...
      if (renderTarget->depthStencilFormat != SF_GRAPHICS_FORMAT_UNDEFINED) {
         ...
         if (renderTarget->depthStencilAttachment) {
            ...
            attachmentDescriptions[depthStencilIndex].format = sfAsVulkanFormat(renderTarget->depthStencilFormat);
            attachmentDescriptions[depthStencilIndex].samples = sfAsVulkanSampleCount(renderTarget->samples);
            ...
            attachmentDescriptions[depthStencilIndex].stencilStoreOp = VK_ATTACHMENT_STORE_OP_STORE;
            attachmentDescriptions[depthStencilIndex].initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
            attachmentDescriptions[depthStencilIndex].finalLayout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;

            depthStencilReference.attachment = depthStencilIndex;
            depthStencilReference.layout = VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL;

            attachmentImageViews[depthStencilIndex] = ((SfGraphicsTexture *)renderTarget->depthStencilAttachment)->vulkanImageView;
         }
      }
      ...
      if (sfValidateGraphicsRenderTargetAttachments(renderTarget)) {
         VkRenderPassCreateInfo const renderPassInfo = {
            ...
            .pSubpasses = &(VkSubpassDescription){
               ...
               .pDepthStencilAttachment = renderTarget->depthStencilFormat != SF_GRAPHICS_FORMAT_UNDEFINED ? &depthStencilReference : NULL,
         ...
         if (SF_VULKAN_CHECK(vkCreateRenderPass(renderer->vulkanDevice, &renderPassInfo, renderer->vulkanAllocationCallbacks, &renderTarget->vulkanRenderPass))) {
            ...
            SF_VULKAN_CHECK(vkCreateFramebuffer(renderer->vulkanDevice, &framebufferInfo, renderer->vulkanAllocationCallbacks, &renderTarget->vulkanFramebuffer));
         }
      }

      if (!sfValidateGraphicsRenderTargetAttachments(renderTarget) || !renderTarget->vulkanRenderPass || !renderTarget->vulkanFramebuffer) {
...
SF_INTERNAL VkShaderModule sfCreateVulkanShaderModule(SfGraphicsRenderer *renderer, uint32_t codeSizeInBytes, void const *code) {
   ...
   if (renderer && codeSizeInBytes && code) {
      ...
      SF_VULKAN_CHECK(vkCreateShaderModule(renderer->vulkanDevice, &info, renderer->vulkanAllocationCallbacks, &shader));
   }
   ...
   if (renderer && programHandle) {
      ...
      if (renderer->vulkanDevice) {
         if (program->vulkanComputeShader) {
            vkDestroyShaderModule(renderer->vulkanDevice, program->vulkanComputeShader, renderer->vulkanAllocationCallbacks);
            program->vulkanComputeShader = VK_NULL_HANDLE;
         }

         if (program->vulkanFragmentShader) {
            vkDestroyShaderModule(renderer->vulkanDevice, program->vulkanFragmentShader, renderer->vulkanAllocationCallbacks);
            program->vulkanFragmentShader = VK_NULL_HANDLE;
         }

         if (program->vulkanGeometryShader) {
            vkDestroyShaderModule(renderer->vulkanDevice, program->vulkanGeometryShader, renderer->vulkanAllocationCallbacks);
            program->vulkanGeometryShader = VK_NULL_HANDLE;
         }

         if (program->vulkanTessellationEvaluationShader) {
            vkDestroyShaderModule(renderer->vulkanDevice, program->vulkanTessellationEvaluationShader, renderer->vulkanAllocationCallbacks);
            program->vulkanTessellationEvaluationShader = VK_NULL_HANDLE;
         }

         if (program->vulkanTessellationControlShader) {
            vkDestroyShaderModule(renderer->vulkanDevice, program->vulkanTessellationControlShader, renderer->vulkanAllocationCallbacks);
            program->vulkanTessellationControlShader = VK_NULL_HANDLE;
         }

         if (program->vulkanVertexShader) {
            vkDestroyShaderModule(renderer->vulkanDevice, program->vulkanVertexShader, renderer->vulkanAllocationCallbacks);
            program->vulkanVertexShader = VK_NULL_HANDLE;
         }
      }
   }
...
SfHandle sfCreateGraphicsProgram(SfGraphicsRenderer *renderer, SfGraphicsProgramDescription const *description) {
   ...
   if (program) {
      if (description->vertexCodeSize) {
         ...
         program->vulkanVertexShader = sfCreateVulkanShaderModule(renderer, description->vertexCodeSize, description->vertexCode);
      }

      if (description->tessellationControlCodeSize) {
         program->stageFlags |= SF_GRAPHICS_SHADER_STAGE_TESSELLATION_CONTROL;
         program->vulkanTessellationControlShader = sfCreateVulkanShaderModule(renderer, description->tessellationControlCodeSize, description->tessellationControlCode);
      }

      if (description->tessellationEvaluationCodeSize) {
         program->stageFlags |= SF_GRAPHICS_SHADER_STAGE_TESSELLATION_EVALUATION;
         program->vulkanTessellationEvaluationShader = sfCreateVulkanShaderModule(renderer, description->tessellationEvaluationCodeSize, description->tessellationEvaluationCode);
      }

      if (description->geometryCodeSize) {
         program->stageFlags |= SF_GRAPHICS_SHADER_STAGE_GEOMETRY;
         program->vulkanGeometryShader = sfCreateVulkanShaderModule(renderer, description->geometryCodeSize, description->geometryCode);
      }

      if (description->fragmentCodeSize) {
         program->stageFlags |= SF_GRAPHICS_SHADER_STAGE_FRAGMENT;
         program->vulkanFragmentShader = sfCreateVulkanShaderModule(renderer, description->fragmentCodeSize, description->fragmentCode);
      }

      if (description->computeCodeSize) {
         program->stageFlags |= SF_GRAPHICS_SHADER_STAGE_COMPUTE;
         program->vulkanComputeShader = sfCreateVulkanShaderModule(renderer, description->computeCodeSize, description->computeCode);
      }

      if ((program->stageFlags & SF_GRAPHICS_SHADER_STAGE_VERTEX) ? !program->vulkanVertexShader : SF_FALSE
         || (program->stageFlags & SF_GRAPHICS_SHADER_STAGE_TESSELLATION_CONTROL) ? !program->vulkanTessellationControlShader : SF_FALSE
         || (program->stageFlags & SF_GRAPHICS_SHADER_STAGE_TESSELLATION_EVALUATION) ? !program->vulkanTessellationEvaluationShader : SF_FALSE
         || (program->stageFlags & SF_GRAPHICS_SHADER_STAGE_GEOMETRY) ? !program->vulkanGeometryShader : SF_FALSE
         || (program->stageFlags & SF_GRAPHICS_SHADER_STAGE_FRAGMENT) ? !program->vulkanFragmentShader : SF_FALSE
         || (program->stageFlags & SF_GRAPHICS_SHADER_STAGE_COMPUTE) ? !program->vulkanComputeShader : SF_FALSE) {
   ...
   if (renderer && descriptorSetHandle) {
      ...
      if (renderer->vulkanDevice) {
         if (descriptorSet->vulkanDescriptorPool && descriptorSet->vulkanDescriptorSet) {
            vkFreeDescriptorSets(renderer->vulkanDevice, descriptorSet->vulkanDescriptorPool, 1, &descriptorSet->vulkanDescriptorSet);
            descriptorSet->vulkanDescriptorSet = VK_NULL_HANDLE;
         }

         if (descriptorSet->vulkanDescriptorPool) {
            vkDestroyDescriptorPool(renderer->vulkanDevice, descriptorSet->vulkanDescriptorPool, renderer->vulkanAllocationCallbacks);
            descriptorSet->vulkanDescriptorPool = VK_NULL_HANDLE;
         }

         if (descriptorSet->vulkanDescriptorSetLayout) {
            vkDestroyDescriptorSetLayout(renderer->vulkanDevice, descriptorSet->vulkanDescriptorSetLayout, renderer->vulkanAllocationCallbacks);
            descriptorSet->vulkanDescriptorSetLayout = VK_NULL_HANDLE;
         }
      }
   }
...
SfHandle sfCreateGraphicsDescriptorSet(SfGraphicsRenderer *renderer, SfGraphicsDescriptorSetDescription const *description) {
   ...
   if (descriptorSet) {
      ...
      if (SF_VULKAN_CHECK(vkCreateDescriptorSetLayout(renderer->vulkanDevice, &layoutInfo, renderer->vulkanAllocationCallbacks, &descriptorSet->vulkanDescriptorSetLayout))) {
         ...
         if (SF_VULKAN_CHECK(vkCreateDescriptorPool(renderer->vulkanDevice, &descriptorPoolInfo, renderer->vulkanAllocationCallbacks, &descriptorSet->vulkanDescriptorPool))) {
            ...
            SF_VULKAN_CHECK(vkAllocateDescriptorSets(renderer->vulkanDevice, &descriptorSetInfo, &descriptorSet->vulkanDescriptorSet));
         }
      }

      if (!descriptorSet->vulkanDescriptorSetLayout || !descriptorSet->vulkanDescriptorPool || !descriptorSet->vulkanDescriptorSet) {
...
SF_INTERNAL VkShaderModule sfGetCorrespondingGraphicsShaderModule(SfGraphicsProgram const *program, SfGraphicsShaderStageFlags stage) {
   switch (stage) {
      case SF_GRAPHICS_SHADER_STAGE_VERTEX:
         return program->vulkanVertexShader;
      case SF_GRAPHICS_SHADER_STAGE_TESSELLATION_CONTROL:
         return program->vulkanTessellationControlShader;
      case SF_GRAPHICS_SHADER_STAGE_TESSELLATION_EVALUATION:
         return program->vulkanTessellationEvaluationShader;
      case SF_GRAPHICS_SHADER_STAGE_GEOMETRY:
         return program->vulkanGeometryShader;
      case SF_GRAPHICS_SHADER_STAGE_FRAGMENT:
         return program->vulkanFragmentShader;
      case SF_GRAPHICS_SHADER_STAGE_COMPUTE:
         return program->vulkanComputeShader;
      default:
...
SF_INTERNAL SfVulkanShaderStageList sfCreateVulkanShaderStageList(SfArena *arena, SfGraphicsPipelineDescription const *description) {
   ...
   if (result.data) {
      ...
      for (uint32_t i = 0; i < 5; ++i) {
         ...
         if (stage & program->stageFlags) {
            ...
            result.data[stage_count].module = sfGetCorrespondingGraphicsShaderModule(program, stage);
...
SF_INTERNAL SfVulkanVertexInputList sfCreateVulkanVertexInputList(SfArena *arena, SfGraphicsPipelineDescription const *description) {
   ...
   if (description->vertexLayout.count < SF_GRAPHICS_MAX_VERTEX_ATTRIBUTE_COUNT) {
      ...
      result.attributes = sfAllocate(arena, SF_GRAPHICS_MAX_VERTEX_ATTRIBUTE_COUNT * sizeof(*result.attributes));
      if (result.bindings && result.attributes) {
         ...
         for (uint32_t i = 0; i < description->vertexLayout.count; ++i) {
            ...
            if (result.bindingCount && result.bindingCount < SF_GRAPHICS_MAX_VERTEX_BINDING_COUNT) {
               result.bindings[result.bindingCount - 1].binding = attribute->binding;
               result.bindings[result.bindingCount - 1].inputRate = VK_VERTEX_INPUT_RATE_VERTEX;
               result.bindings[result.bindingCount - 1].stride = sfGetGraphicsFormatStride(attribute->format);
...
SF_INTERNAL VkPrimitiveTopology sfAsVulkanPrimitiveTopology(SfGraphicsPrimitiveTopology topology) {
   switch (topology) {
      case SF_GRAPHICS_PRIMITIVE_TOPOLOGY_POINT_LIST:
         return VK_PRIMITIVE_TOPOLOGY_POINT_LIST;
      case SF_GRAPHICS_PRIMITIVE_TOPOLOGY_LINE_LIST:
         return VK_PRIMITIVE_TOPOLOGY_LINE_LIST;
      case SF_GRAPHICS_PRIMITIVE_TOPOLOGY_LINE_STRIP:
         return VK_PRIMITIVE_TOPOLOGY_LINE_STRIP;
      case SF_GRAPHICS_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST:
         return VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;
      case SF_GRAPHICS_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP:
         return VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP;
      case SF_GRAPHICS_PRIMITIVE_TOPOLOGY_TRIANGLE_FAN:
      ...
      case SF_GRAPHICS_PRIMITIVE_TOPOLOGY_PATCH_LIST:
...
SF_INTERNAL VkCullModeFlags sfAsVulkanCullMode(SfGraphicsCullMode cullMode) {
   switch (cullMode) {
      case SF_GRAPHICS_CULL_MODE_NONE:
         return VK_CULL_MODE_NONE;
      case SF_GRAPHICS_CULL_MODE_FRONT:
         return VK_CULL_MODE_FRONT_BIT;
      case SF_GRAPHICS_CULL_MODE_BACK:
         return VK_CULL_MODE_BACK_BIT;
      case SF_GRAPHICS_CULL_MODE_FRONT_AND_BACK:
...
SF_INTERNAL VkFrontFace sfAsVulkanFrontFace(SfGraphicsFrontFace frontFace) {
   switch (frontFace) {
      case SF_GRAPHICS_FRONT_FACE_CLOCKWISE:
         return VK_FRONT_FACE_CLOCKWISE;
      case SF_GRAPHICS_FRONT_FACE_COUNTER_CLOCKWISE:
...
SfHandle sf_graphics_create_pipeline(SfGraphicsRenderer *renderer, SfGraphicsPipelineDescription const *description) {
   ...
   if (pipeline) {
      ...
      SfGraphicsDescriptorSet const *descriptorSet = (SfGraphicsDescriptorSet const *)description->descriptorSet;
      ...
      if (SF_VULKAN_CHECK(vkCreatePipelineLayout(renderer->vulkanDevice, &layout_info, renderer->vulkanAllocationCallbacks, &pipeline->vulkanPipelineLayout))) {
         SfVulkanShaderStageList const stages = sfCreateVulkanShaderStageList(&renderer->arena, description);
         SfVulkanVertexInputList const input = sfCreateVulkanVertexInputList(&renderer->arena, description);
         SfGraphicsRenderTarget const* renderTarget = (SfGraphicsRenderTarget const*)description->renderTarget;
         if (stages.count && stages.data && input.bindingCount && input.bindings && input.attributeCount && input.attributes) {
            ...
            VkPipelineTessellationDomainOriginStateCreateInfoKHR const tessellationDomainOriginInfo = {
            ...
            VkGraphicsPipelineCreateInfo const pipelineInfo = {
               ...
               .pTessellationState = (description->topology == SF_GRAPHICS_PRIMITIVE_TOPOLOGY_PATCH_LIST) ? &tessellationInfo : NULL,
            ...
            SF_VULKAN_CHECK(vkCreateGraphicsPipelines(renderer->vulkanDevice, VK_NULL_HANDLE, 1, &pipelineInfo, renderer->vulkanAllocationCallbacks, &pipeline->vulkanPipeline));
         }
      }
   }
...
SfHandle sfGetGraphicsQueueHandle(SfGraphicsRenderer *renderer) {
   return SF_AS_HANDLE(&renderer->graphicsQueue);
}

SfHandle sfGetPresentQueueHandle(SfGraphicsRenderer *renderer) {
   return SF_AS_HANDLE(&renderer->presentQueue);
}


SF_INTERNAL SfBool sfTestVulkanFormatFeatures(SfGraphicsRenderer *renderer, VkFormat format, VkImageTiling tiling, VkFormatFeatureFlags features) {
   SfBool result = SF_FALSE;
   if (renderer) {
   ...
   if (renderer) {
      ...
      if (renderer->arena.data) {
         ...
         VkInstanceCreateInfo const instanceInfo = {
            ...
            .pNext = NULL,
            .flags = 0 | ((renderer->plataform.type == SF_PLATAFORM_TYPE_MACOS) ? VK_INSTANCE_CREATE_ENUMERATE_PORTABILITY_BIT_KHR : 0),
         ...
         if (SF_VULKAN_CHECK(vkCreateInstance(&instanceInfo, renderer->vulkanAllocationCallbacks, &renderer->vulkanInstance))) {
            renderer->vulkanCreateDebugUtilsMessengerEXT = SF_VULKAN_PROC(vkCreateDebugUtilsMessengerEXT, renderer->vulkanInstance);
            renderer->vulkanDestroyDebugUtilsMessengerEXT = SF_VULKAN_PROC(vkDestroyDebugUtilsMessengerEXT, renderer->vulkanInstance);

            if (renderer->vulkanCreateDebugUtilsMessengerEXT && renderer->vulkanDestroyDebugUtilsMessengerEXT) {
               ...
               PFN_vkCreateDebugUtilsMessengerEXT const create = renderer->vulkanCreateDebugUtilsMessengerEXT;
               SF_VULKAN_CHECK(create(renderer->vulkanInstance, &validatorInfo, renderer->vulkanAllocationCallbacks, &renderer->vulkanValidationMessenger));
            }

            if (SF_VULKAN_CHECK(glfwCreateWindowSurface(renderer->vulkanInstance, renderer->plataform.window, renderer->vulkanAllocationCallbacks, &renderer->vulkanSurface))) {
               SfVulkanPhysicalDeviceList deviceList = sfVulkanCreatePhysicalDeviceList(&renderer->arena, renderer);

               for (uint32_t i = 0; i < deviceList.count && !renderer->vulkanPhysicalDevice; ++i) {
            ...
            if (renderer->vulkanSurface && renderer->vulkanPhysicalDevice) {
               SfVulkanSurfaceFormatList surfaceFormatList = sfVulkanCreateSurfaceFormatList(&renderer->arena, renderer);
               if (surfaceFormatList.count && surfaceFormatList.data) {

                  renderer->vulkanSurfaceFormat = surfaceFormatList.data[0].format;
                  renderer->vulkanSurfaceColorSpace = surfaceFormatList.data[0].colorSpace;

                  for (uint32_t i = 0; i < surfaceFormatList.count; ++i) {
                     VkSurfaceFormatKHR *format = &surfaceFormatList.data[i];

                     if (format->format == VK_FORMAT_B8G8R8A8_SRGB || format->colorSpace == VK_COLOR_SPACE_SRGB_NONLINEAR_KHR) {
                        renderer->vulkanSurfaceFormat = VK_FORMAT_B8G8R8A8_SRGB;
                        renderer->vulkanSurfaceColorSpace = VK_COLOR_SPACE_SRGB_NONLINEAR_KHR;
                        break;
                     }
                  }
               }
            ...
            if (renderer->vulkanSurface && renderer->vulkanPhysicalDevice) {
               SfVulkanPresentModeList presentModeList = sfVulkanCreatePresentModeList(&renderer->arena, renderer);
               if (presentModeList.count && presentModeList.data) {
                  ...
                  for (uint32_t i = 0; i < presentModeList.count && !foundPresentMode; ++i) {
            ...
            if(renderer->vulkanSurface && renderer->vulkanPhysicalDevice) {
               if (sf_vulkan_test_format_features(renderer, VK_FORMAT_D32_SFLOAT, VK_IMAGE_TILING_OPTIMAL, VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT))
                  renderer->vulkanDepthStencilFormat = VK_FORMAT_D32_SFLOAT;
               else if (sf_vulkan_test_format_features(renderer, VK_FORMAT_D32_SFLOAT_S8_UINT, VK_IMAGE_TILING_OPTIMAL, VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT))
                  renderer->vulkanDepthStencilFormat = VK_FORMAT_D32_SFLOAT_S8_UINT;
               else if (sf_vulkan_test_format_features(renderer, VK_FORMAT_D24_UNORM_S8_UINT, VK_IMAGE_TILING_OPTIMAL, VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT))
            ...
            if (renderer->vulkanPhysicalDevice) {
               ...
               VkDeviceCreateInfo const device_info = {
                  ...
                  .pNext = NULL,
                  .flags = 0,
                  .queueCreateInfoCount = (renderer->graphicsQueue.vulkanQueueFamilyIndex == renderer->presentQueue.vulkanQueueFamilyIndex) ? 1 : SF_SIZE(queue_info),
               ...
               if (SF_VULKAN_CHECK(vkCreateDevice(renderer->vulkanPhysicalDevice, &device_info, renderer->vulkanAllocationCallbacks, &renderer->vulkanDevice))) {
                  vkGetDeviceQueue(renderer->vulkanDevice, renderer->graphicsQueue.vulkanQueueFamilyIndex, 0, &renderer->graphicsQueue.vulkanQueue);
                  vkGetDeviceQueue(renderer->vulkanDevice, renderer->presentQueue.vulkanQueueFamilyIndex, 0, &renderer->presentQueue.vulkanQueue);
               }
            }
   ...
   if (renderer) {
      ...
      if (renderer->vulkanInstance) {
         if (renderer->vulkanSurface) {
            vkDestroySurfaceKHR(renderer->vulkanInstance, renderer->vulkanSurface, renderer->vulkanAllocationCallbacks);
            renderer->vulkanSurface = VK_NULL_HANDLE;
         }
         if (renderer->vulkanDestroyDebugUtilsMessengerEXT && renderer->vulkanValidationMessenger) {
            renderer->vulkanDestroyDebugUtilsMessengerEXT(renderer->vulkanInstance, renderer->vulkanValidationMessenger, renderer->vulkanAllocationCallbacks);
