...
#define SF_NULL_HANDLE (SFHandle){ .value = 0 }
...
typedef struct SFGraphicsGLFWPlatform {
  GLFWwindow *window;
  I32         windowWidth;
  I32         windowHeight;
...
typedef struct SFGraphicsDescriptorSetLayout {
  U32                  descriptorCount;
  SFGraphicsDescriptor descriptors[SF_GRAPHICS_MAX_DESCRIPTOR_SET_DESCRIPTOR_COUNT];
} SFGraphicsDescriptorSetLayout;
...
typedef void (*SFGraphicsRendererCallback)(void *data, void *renderer);
...
typedef struct SFGraphicsRendererDescription {
  ...
  U32                        width;
  U32                        height;
  SFGraphicsRendererCallback createVulkanSurface;
  SFGraphicsRendererCallback requestSwapchainDimensions;
  SFString                   applicationName;
  U32                        vkInstanceExtensionCount;
  char const               **vkInstanceExtensions;
  U32                        vkInstanceLayerCount;
  char const               **vkInstanceLayers;
  U32                        vkDeviceExtensionCount;
  char const               **vkDeviceExtensions;
...
typedef struct SFGraphicsRenderer {
  ...
  SFGraphicsTexture                   texturePool[SF_GRAPHICS_MAX_RESOURCE_POOL_COUNT];
  SFGraphicsRenderTarget              renderTargetPool[SF_GRAPHICS_MAX_RESOURCE_POOL_COUNT];
  SFGraphicsCommandBuffer             commandBufferPool[SF_GRAPHICS_MAX_RESOURCE_POOL_COUNT];
  SFGraphicsDescriptorSet             descriptorSetPool[SF_GRAPHICS_MAX_RESOURCE_POOL_COUNT];
  SFGraphicsPipeline                  pipelinePool[SF_GRAPHICS_MAX_RESOURCE_POOL_COUNT];
} SFGraphicsRenderer;
