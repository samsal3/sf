...
#define SF_STRING_FROM_LITERAL(s, literal)                                                                            \
	do {                                                                                                          \
		(s)->data = &(literal)[0];                                                                            \
...
#define SF_MEMORY_SET(destination, value, count)                                                                      \
	do {                                                                                                          \
		uintptr_t i_;                                                                                         \
		for (i_ = 0; i_ < (count); ++i_)                                                                      \
			((char *)(destination))[i_] = (char)(value);                                                  \
	} while (0)
...
SF_EXTERNAL void sf_string_null_terminate(struct sf_arena *arena, struct sf_string *source, struct sf_string *result);
